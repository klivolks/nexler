...
// kpassPermissionTmpl renders middleware/permission_kpass.go — wires
// middleware.PermissionCheck to services/kpass.CheckAccess, generated by
// NewKpass only when the target app already has the Task Registry
// (middleware/task.go). See kpass.go's NewKpass and
// ensureKpassPermissionHook.
const kpassPermissionTmpl = "kpass_templates/permission_kpass.go.tmpl"
...
// tenantTemplateFS embeds the tenant-org integration templates used by
// `nexler init tenant`, same dependency-free principle as templateFS.
//
//go:embed tenant_templates
var tenantTemplateFS embed.FS

const (
	tenantTmpl         = "tenant_templates/tenant.go.tmpl"
	tenantHandlersTmpl = "tenant_templates/tenant_handlers.go.tmpl"
)
